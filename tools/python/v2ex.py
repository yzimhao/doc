...
file_handler = logging.FileHandler("/tmp/v2ex.log")
...
opt.add_option('--username', '-u', default="usrname")
opt.add_option('--password', '-p', default="password")
...
class V2EX:
    ...
    def login(self):
        ...
        username_tag = v2ex_main_tag.find(
            'input', attrs={"class": "sl", "placeholder": u"用户名或电子邮箱地址"}
        )
        password_tag = v2ex_main_tag.find(
            'input', attrs={"type": "password"}
        )
        tag_u_name = username_tag.attrs[2][1]
        tag_p_name = password_tag.attrs[2][1]

        input_once_tag = form_tag.find('input', attrs={"name": "once"})
        input_once_value = input_once_tag.attrs[1][1]
        login_info = {
            "next": "/",
            tag_u_name: self.username,
            tag_p_name: self.password,
    ...
    def checkin(self):
        ...
        award_req = self.v2ex_session.get(
        ...
        # return
        get_award_soup = BeautifulSoup.BeautifulSoup(award_req.content)
        button_tag = get_award_soup.find('input', attrs={'type': 'button'})
        click_href = button_tag.attrs[3][1].replace("location.href = '", '').replace("'", '')
        # print click_href
        click_url = "http://www.v2ex.com" + click_href
        # print click_url
...
if __name__ == '__main__':
    v2ex = V2EX(options.username, options.password)
    v2ex.run()
